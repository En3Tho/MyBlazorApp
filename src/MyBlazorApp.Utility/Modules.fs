...
open MyBlazorApp.Utility.Disposables
...
module Option =
    ...
    let inline ofBool bool = if bool then unitSome else None
...
module String =
    ...
    let inline (|Contains|_|) (pattern: string) (str: string) = str.Contains pattern |> Option.ofBool
...
module Tuple =
    /// returns both result of computation and original value in tupled form
    let inline ofResVal f x = f x, x

    /// returns both original value and result of computation in tupled form
    let inline ofValRes f x = x, f x
       ...
       static member generateGetTag() =
            ...
            if parameterType |> FSharpType.IsUnion |> not
               ...
               || isNull tagPropertyInfo.GetMethod then
                invalidOp $"Invalid type specified: %s{parameterType.FullName}"
            else
